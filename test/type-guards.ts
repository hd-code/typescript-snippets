import { hasKey, isArray, isBool, isInteger, isNull, isNumber, isObject, isString, isUndefined } from 'type-guards';
...
describe('TypeGuards', () => {
    ...
    describe(isNumber.name, () =>
        checkCases(isNumber, ['zero', 'integer', 'integerNegative', 'number', 'numberNegative']),
    );
    ...
    describe(hasKey.name, () => {
        it.each([
            ...
            ['object has key and type guard matches', true, { name: 'John' }, 'name', isString],
            ['object has key but type guard fails', false, { name: 'John' }, 'name', isNumber],
            ['object has key with falsy value (empty string)', true, { name: '' }, 'name', undefined],
            ['object has key with falsy value (0)', true, { name: 0 }, 'name', undefined],
            ['object has key with falsy value (null)', true, { name: null }, 'name', undefined],
            ['object has key with falsy value (undefined)', true, { name: undefined }, 'name', undefined],
