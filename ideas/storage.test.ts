    ...
    describe(msg, () => {
        ...
        describe("get()", () => {
            it("should return the referenced entry", () => {
                const cache = initFilledStorage();
                assert.deepEqual(cache.get(id2), person2);
            });

            it("should return undefined for a non-existing id", () => {
                const cache = initFilledStorage();
                assert.deepEqual(cache.get("Do I exist?"), undefined);
            });
        });

        it("getAll()", () => {
            const cache = initFilledStorage();
            assert.deepEqual(cache.getAll(), filledStorage);
        });
        ...
        describe("set()", () => {
            ...
            it("should return the former element for an existing id", () => {
                ...
                assert.deepEqual(old, person1, "did not return former element");
                assert.deepEqual(
                    cache.get(id1),
                    person3,
