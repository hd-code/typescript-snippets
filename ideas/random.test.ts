...
describe("random", () => {
        ...
        [
            {
                ...
                expected: { from: 0, til: 1 },
            },
            {
                name: "with max",
                args: [10, undefined],
                expected: { from: 0, til: 10 },
            },
            {
                name: "with negative max",
                args: [-10, undefined],
                expected: { from: -10, til: 0 },
            },
            {
                name: "with min and max",
                args: [5, 15],
                expected: { from: 5, til: 15 },
            },
            {
                name: "with negative min",
                args: [-5, 15],
                expected: { from: -5, til: 15 },
            },
            {
            ...
            },
            {
                ...
                args: [-20, -10],
                expected: { from: -20, til: -10 },
            },
            {
                name: "negative min and max wrong order",
                args: [-10, -20],
                expected: { from: -20, til: -10 },
            },
            {
            ...
            it(name, () => {
                ...
                for (let i = 0; i < numberOfTries; i++) {
                    actuals.push(
                        getFloat(args[0] as number, args[1] as number)
                    );
                }
                ...
                assert.ok(
                    smallest >= expected.from,
                    "some value were too small"
                );
                assert.ok(smallest < expected.til, "some value were too large");
                assert.ok(
                    largest >= expected.from,
                    "some value were too small"
                );
    ...
    describe(getInt.name, () => {
        [
            ...
            {
                name: "negative real min and max",
                args: [-20.5, -10.2],
                expected: { from: -20, til: -10 },
            },
            {
            ...
            it(name, () => {
                ...
                assert.ok(
                    smallest >= expected.from,
                    "smallest value was too small"
                );
                assert.ok(
                    smallest <= expected.til,
                    "smallest value was too large"
                );
                assert.ok(
                    largest >= expected.from,
                    "largest value was too small"
                );
                assert.ok(
                    largest <= expected.til,
                    "largest value was too large"
                );
    ...
    describe(setSeed.name, () => {
        ...
        it("should generate different values for 0 and 1", () => {
            ...
            assert.notDeepEqual(valuesZero, valuesOne);
        });
    });
});
